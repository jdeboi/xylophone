...
int numKeys = 6;
int baudRate = 57600;
int [] PadNote = {60,62,64,65,67,69};	            //array containing MIDI note values,                                                           
int [] PadCutOff = {50,50,50,50,50,50};	    //array containing "threshold" values; up to 1023, but probably around 600                                                            //increase (up to 1023) in order to make piezos less sensitive  
int [] MaxPlayTime = {90,90,90,90,90,90};		    //"time" each note remains active												
int [] activePad = {0,0,0,0,0,0};                       // array of flags of pad currently playing
int [] PinPlayTime = {0,0,0,0,0,0};                     // counter since pad started to play
...
void readSensors() {
  for (count = 0; count< numKeys; count++) {
      notes[count] = arduino.analogRead(count);	  //each element of array "notes" corresponds to the  
      //if(notes[n] > PadCutOff[n]){
        println((count+1) + ": " + notes[count]);            //use to print the voltage (0 to 1023 corresponding to 0 to 5V)  
      //}
...
void checkSensor(int i){		//function to get values of each piezo; only checking a single analog input pin
  hitavg = notes[i];			//variable hitavg equals the voltage (0-1023) of the piezo
